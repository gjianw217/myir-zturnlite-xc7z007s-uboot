...
static int do_get_bitstream_name(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	// printf("device_code is : %d\n\n\n", device_code);

	switch(device_code) {
	case ZYNQ_7Z007:
		return env_set("bitstream_image", "7z007.bit");

	case ZYNQ_7Z010:
		return env_set("bitstream_image", "7z010.bit");
		
	case ZYNQ_7Z015:
		return env_set("bitstream_image", "7z015.bit");

	case ZYNQ_7Z020:
		return env_set("bitstream_image", "7z020.bit");
...
static int do_qspi_get_bitsize(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	rc = env_set_hex("bitsize", bitsize);
	if (rc) {
